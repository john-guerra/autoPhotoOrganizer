...
var albumsChart = require("./albumsChart.js");
var photoTimelineChart = require("./photoTimelineChart.js");
var d3 = require("d3");
...
var timelineC = photoTimelineChart().x(function (d) {
  return d.createDate;
});
...
function update(_photos) {
  ...
  var temp = d3.select("#chart").datum(data);
  temp.call(timelineC);
...
function updateAlbums(_albums) {
  ...
  var temp = d3.select("#albums").datum(albums);
  temp.call(albumsC);
}
...
function reorderPhotos(photos) {
  ...
  if (n < 3) {
    ...
    // Reassemble the list
    return [photos[0], photos[n - 1], photos[pivot]].concat(left).concat(right);
  }
}
...
function handleFileSelect(evt) {
  ...
  for (i = 0; (f = files[i]); i++) {
    ...
    if (
      !d3.select("#chIncludeAllFiles").property("checked") &&
      !f.type.match("image.*") &&
      !f.type.match("video.*")
    ) {
      continue;
  ...
  if (toFrom === "to") {
    switch (range) {
      ...
      case "minutes":
        ...
        break;
      case "hours":
        timeConverted = time / (1000 * 60 * 60);
        break;
      case "days":
        timeConverted = time / (1000 * 60 * 60 * 24);
        break;
      case "months":
        timeConverted = time / (1000 * 60 * 60 * 24 * 30);
        break;
      case "years":
        timeConverted = time / (1000 * 60 * 60 * 24 * 365);
        break;
    }
  } else {
    switch (range) {
...
function computeSeparations() {
  ...
  for (i = 0; i < photosByDate.length - 1; i += 1) {
    separations.push(
      photosByDate[i + 1].createDate - photosByDate[i].createDate
    );
  }
  var before = new Date();
...
function copyAlbums() {
  ...
  if (!outputFolder[0]) {
    alert("Select an output folder first. Probably with files");
    return;
  }

  // An object to keep the report of progress of each album
  var progressReportDict = {};

  var reportProgress = function () {
    d3.select("#copyProgress").text(d3.values(progressReportDict).join("\n"));
  };
  ...
  var copyAlbum = function (album) {
    ...
    var isMoving = document.getElementById("chMoveOrCopy").checked;
    var albumOutput = outputFolder[0]
      ? path.join(path.dirname(outputFolder[0].path), album.name)
      : path.dirname(album.photos[0].path);

    console.log("Copying/Moving to " + albumOutput);

    var callbackCopy = function (err) {
      var copyingStr = isMoving ? " moving " : " copying ";
      if (err) {
        alert("Error " + copyingStr + " the file " + err);
        console.log(err);
        return;
      }
      photosCopiedCount += 1;
      progressReportDict[album.name] =
        "Album: " +
        albumOutput +
        copyingStr +
        photosCopiedCount +
    ...
    fse.ensureDir(albumOutput, function (err) {
      if (err) {
        alert("Error creating the album folder " + albumOutput);
        console.log(err);
        return;
      }

      // Then copy each photo
      album.photos.forEach(function (photo) {
        if (isMoving) {
          fse.move(
            photo.url,
            path.join(albumOutput, path.basename(photo.url)),
            callbackCopy
          );
        } else {
          fse.copy(
...
function onChangeMoveOrCopy() {
  var isMoving = document.getElementById("chMoveOrCopy").checked;
  var copyingStr = isMoving ? "Move files" : "Copy Files";
  document.getElementById("btnCopy").textContent = copyingStr;
}
...
d3.select("#chMoveOrCopy").on("change", onChangeMoveOrCopy);
