...
1. **Portability** — the best knowledge lives _outside_ the repo (Claude-only,
...
3. **One monolithic file getting worse** — `App.svelte` is 6,125 lines and _grew 34%_
...
- `CLAUDE.md` is a real harness asset, not boilerplate: it encodes traps _with the
  mechanism_ (`$:`-on-`bind:this` → `safe_not_equal`; `|global` transition
...
- Layered, concrete guardrails: two invariants (folders are truth; SQLite is a
  rebuildable cache), a "never touch the user's real photo folders" rule _stricter_
...
- **Path traversal structurally avoided** — image endpoints serve by DB-looked-up
  `id` (`Number()`-coerced), not user paths; the one user-path _write_ routes through
...
**Portability — the best knowledge isn't in the repo** _(harness gap)_
...
**Doc freshness at the entry points** _(SDD drift)_
...
- `App.svelte` is **6,125 lines and grew ~34%** since the audit that named it the
  ...
  exist _because_ everything lives here.
...
**The headline RAW feature can't be built on the current stack** _(stack-vs-goal)_

- "Never fully decode a RAW during culling" needs an embedded-JPEG-preview
  _extractor_ — the stack has **none** (`exifr.thumbnail()` returns a ~160px thumb and
...
**Accessibility — the keyboard-first app under-serves AT users** _(HCI gap)_
...
**Security & CI process gaps** _(none are live exploits)_

- **No Dependabot, no CodeQL, no scheduled `npm audit` gate** — the manual currency
  discipline has no automated backstop. _(Medium)_
- `safeResolve` uses string-containment, not `realpath` — a symlinked write
  destination could escape the promised root. _(Low; user picks the folder)_
- No CSP; no Electron `setWindowOpenHandler`/`will-navigate` deny. _(Low,
  defense-in-depth)_
...
**Medium (this month)** 6. **Add `AGENTS.md` + committed `.claude/settings.json`, and port durable
`MEMORY.md` items into a checked-in `docs/AGENT-NOTES.md`.** Keep personal paths in
the gitignored local doc. 7. **Give the Loupe real modal semantics** (rebuild on `Modal.svelte`'s `<dialog>` or
add trap + focus-restore) and **fix the grid ARIA contract**. 8. **Add `exiftool-vendored` behind `ProcessingService.extractPreview`** — extract
...
against today). 9. **Resolve the albums architecture drift** (move server-side per README, or delete
the README and correct CLAUDE.md). 10. **Decide content-hash:** finish it (loop on `remaining` behind `whenIdle()`) or
delete the column + its green test. 11. **Broaden `prefers-reduced-motion`**; restore visible `:focus-visible` on ring-less
inputs.

**Larger (prioritize — long-term lever)** 12. **Extract the feed-window state machine + selection/undo out of `App.svelte` into
...
34%/cycle. Do it _surgically_ behind the existing `withFeedTransaction` seam — not
a rewrite.
...
- **Harness thesis** ("port knowledge into the harness") → CLAUDE.md is exemplary, but
  the memory system isn't yet a _repo_ asset. → Rec 6.
...
- **Functional-but-insecure / GitClear code-decay data** → shows here as
  _maintainability_ decay concentrated in `App.svelte`. → Rec 12.
