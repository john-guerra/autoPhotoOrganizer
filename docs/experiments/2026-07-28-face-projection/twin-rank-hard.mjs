...
const MIN_FACES = Number(process.argv[4] ?? 6);
const GAP_H = Number(process.argv[5] ?? 24);
...
const rows = db
  .prepare(
    `SELECT f.id, f.person_id, f.dim, f.scale, f.vec,
            COALESCE(p.taken_at, p.mtime) AS t
       FROM photo_faces f JOIN photos p ON p.id = f.photo_id
      WHERE f.model='buffalo_s' AND f.person_id IS NOT NULL
...
for (const [, faces] of byPerson) {
  if (faces.length >= MIN_FACES) {
    // HARD split: earliest half vs latest half. Different days means
    // different light, clothes and angle -- the reason clustering split the
    // person in the first place. Rows already arrive ordered by time.
    const mid = Math.floor(faces.length / 2);
    const a = faces.slice(0, mid);
    const b = faces.slice(mid);
    if (b[0].t - a[a.length - 1].t < GAP_H * 3600000) continue; // need a real time gap
