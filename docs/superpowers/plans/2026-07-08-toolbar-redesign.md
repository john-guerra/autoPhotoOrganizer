...
describe("applyRatingClick", () => {
  ...
  it("preserves orientations untouched", () => {
    const out = applyRatingClick(
      { minRating: 0, orientations: ["portrait"] },
      2
    );
...
<style>
  ...
  .star.preview {
    color: #7a6a2c;
  } /* dim amber while hovering a higher slot */
</style>
```
...
<header class="topbar">
  ...
  <div class="cluster source">
    <div class="library">
      <button
        ...
        title="Recently scanned folders">Library ▾</button
      >
      {#if libraryOpen}
        <ul class="library-panel">
          <!-- KEEP the existing library-panel <li> contents verbatim -->
        </ul>
      {/if}
    </div>
          ...
          <div class="add-actions">
            ...
            {#if hasNativePicker}
              <button
                ...
                disabled={scanning}>Choose Folder…</button
              >
            {/if}
          </div>
        </div>
  ...
  <div class="cluster view">
    <div class="sidebar-toggle" role="group" aria-label="Sidebar view">
      <!-- KEEP existing Tree/Fisheye buttons -->
    </div>
    <button
      class="reveal-btn"
      on:click={revealCurrentLocation}
      title="Reveal the current photo's location in the tree">⌖ Locate</button
    >
    <div class="view-cell">
