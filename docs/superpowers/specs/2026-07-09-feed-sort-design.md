...
the _same_ intent one level up.

This is foundational and orthogonal to the timeline-date-filter feature, so it is
its own spec. (The timeline spec sets the `month` dimension's _default_ direction;
...
- **Creation date needs a new column.** The schema stores `mtime` (modified) and
  ...
  from the _same_ `stat()` the scanner already calls (`st.birthtimeMs` — free, no
  extra I/O), backfilled on the next rescan.
- **Grouping follows the sort's date source.** The year/month/day dimensions
  re-derive their _column_ (taken / created / modified) and _direction_ from the
  ...
  aggregate of a _non-grouped_ attribute (folders by newest photo / by count) is
  deferred.
...
4. **Date-sort ↔ date-dimension coupling:** the year/month/day group dimensions
   (feed _and_ tree/fisheye sidebars) re-derive their date **column and direction**
   from the current date sort — so grouping by month while sorting by creation date
   buckets by _creation_ month, and month asc/desc is just the sort direction.
...
- Making the **date filter / timeline / albums** honor the taken-vs-created-vs-
  modified choice — grouping _does_ follow the sort's date source (§3), but the
...
determinism only needs to be _total together with the `id` tiebreak_, so ties are
...
Questions — this changes within-_folder_ order from scan-order to date-desc).
...
- **Placeholders / collapsed groups are unaffected.** They live at _group_
  granularity; the sort dim sits _below_ the deepest group dim, so collapsed-group
...
`startPathCondition` (jump-to-tree-node) still seeks to a _group_ position on the
...
are not hardwired to `taken_at` — they re-derive their column _and_ direction from
...
(taken, DESC) and only reorder photos _within_ leaf groups.
...
Filter and grouping are orthogonal: sort changes only _order_, never _membership_
...
- Group by folder, sort by rating → photos reorder _within_ each folder; folder
  order unchanged.
...
2. **Grouping follows the sort's date source** — confirmed. The year/month/day
   ...
   _filter/timeline_ and _albums_ still use `taken_at` (a future unification is
