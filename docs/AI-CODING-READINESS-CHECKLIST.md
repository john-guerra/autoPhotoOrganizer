...
scaffolding that turns a capable model into a reliable contributor to *your* codebase.
...
- [ ] **Agent guide at the repo root.** Add `CLAUDE.md` (and/or `AGENTS.md` — see
  ...
  that matter, and the **traps that cost you an afternoon** (with the *mechanism*, not
...
- [ ] **Explicit guardrails / action boundaries** in the agent guide: what the agent
  must *never* touch (production data, user files, credentials), and destructive-action
...
- [ ] **Keep durable project knowledge *in the repo*, not in a per-user/per-machine
...
- [ ] **Tests are the specification.** Adopt red → green explicitly: write the failing
  test, confirm it fails, implement to green *without editing the test*. Agents default
...
- [ ] **A formatter + linter in CI** so style is never a review topic and diffs stay
  clean for agents. **[S]**
- [ ] **Keep docs honest about code.** When docs drift from code, say so *in the doc*
  and tell the reader to trust the code — a wrong premise sends an agent down a wrong
  path faster than no doc at all. **[S]**

### User experience & accessibility
- [ ] **Never fail silently** — every user-triggered failure surfaces a *visible,
  specific, actionable* message; a console error is not user feedback. **[M]**
...
- [ ] **Adopt an SDD loop** appropriate to your maturity (Fowler's **spec-first →
  spec-anchored → spec-as-source**). A practical shape: `specs/` (the durable *why*),
...
- [ ] **Make spec↔code citations bidirectional** — source files cite their spec; the
  ...
  *current* (a stale entry point is worse than none). **[M]**
...
*Principle to remember: add harness only where it encodes something the model can't yet
